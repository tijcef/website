...
function PolicyPage({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <SimplePage eyebrow="Governance and Accountability" title={title} metaDescription={description} image={researchImage}><div className="prose prose-lg max-w-4xl">{children}<p><strong>Contact:</strong> <a href="mailto:info@tijcef.org">info@tijcef.org</a></p><p><strong>Last reviewed:</strong> 25 August 2026.</p></div></SimplePage>;
}
...
export function Accessibility() {
  return <PolicyPage title="Accessibility Statement" description="TIJCEF's commitment to an inclusive and accessible website."><p>TIJCEF is committed to making its website and digital resources usable by the widest possible audience, including people who use assistive technologies.</p><h2>Our approach</h2><ul><li>Use clear headings, meaningful link text and keyboard-accessible controls.</li><li>Provide text alternatives for informative images.</li><li>Maintain readable colour contrast and responsive text.</li><li>Respect reduced-motion preferences where supported.</li><li>Review forms and important journeys for accessible labels and status messages.</li></ul><h2>Requesting support</h2><p>If a page, form or document is difficult to use, tell us what you were trying to access and the format that would work better. We will make a reasonable effort to provide the information in an accessible form.</p></PolicyPage>;
}
